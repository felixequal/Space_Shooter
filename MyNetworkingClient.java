...
private void createSagePhysicsWorld()
	...
	/*
	 * float massCube = 1000.0f; for (PhysCube pcube: physCubeList){ cubeP =
	 * physicsEngine
	 * .addBoxObject(physicsEngine.nextUID(),massCube,pcube.getCube
	 * ().getWorldTransform().getValues(), pcube.getSize());
	 * cubeP.setBounciness(1.0f); pcube.getCube().setPhysicsObject(cubeP); }
	 */
	float massShip = 5.0f;
...
public void initGameObjects()
	...
	/*
	 * for (int p = 0; p < 15; p++) { Random r = new Random(); double a =
	 * rangeMin + (rangeMax - rangeMin) * r.nextDouble(); double b =
	 * rangeMin + (rangeMax - rangeMin) * r.nextDouble(); double c =
	 * rangeMin + (rangeMax - rangeMin) * r.nextDouble(); float x = (float)
	 * (a / 3) * 1.5f; float y = (float) (b / 4) * 1.5f; float z = (float)
	 * (c / 3) * 1.5f; Matrix3D localTranslation = new Matrix3D();
	 * localTranslation.translate(a, b, c); cube = new Cube(); cube.scale(x
	 * ,y ,z); cube.setWorldTranslation(localTranslation);
	 * cube.updateWorldBound(); physCube = new PhysCube(x,y,z,cube);
	 * physCubeList.add(physCube); addGameWorldObject(cube); }
	 */
