...
public class MyClient extends GameConnectionClient
	...
	protected void processPacket(Object msg)
		...
		if (msgTokens[0].compareTo("move") == 0)
			{
			ghostID = UUID.fromString(msgTokens[1]);
			// System.out.println("Received move message from server from: " +
			// ghostID);
			x = Double.parseDouble(msgTokens[2]);
			y = Double.parseDouble(msgTokens[3]);
			z = Double.parseDouble(msgTokens[4]);
			Vector3D ghostcrVector = new Vector3D(x, y, z);
			// System.out.println("move vector from other client: " +
			// ghostVector.toString());
			for (GhostAvatar check : ghostAvatars)
				{
				if (check.getGhostID().equals(ghostID))
					{
					// System.out.println("found correct ghost. Moving: " +
					// ghostID);
	...
	private void removeGhostAvatar(UUID ghostID)
		...
		int gAIndex = -1;
		/*
		 * Iterator iter = game.getGameWorld().iterator(); while(iter.hasNext())
		 * { if (iter.next() instanceof GhostAvatar) GhostAvatar instance =
		 * node; if (instance.getGhostID() == ghostID)
